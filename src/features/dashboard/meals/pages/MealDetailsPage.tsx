...
function TrendSparkline({ points, color = '#2563eb', label }: TrendSparklineProps): JSX.Element {
  ...
  return (
    ...
    >
      ...
      {points.length ? (
        <>
          <circle cx={0} cy={max === min ? height / 2 : height - ((points[0] - min) / (max - min)) * height} r={3} fill={color} />
          <circle
            cx={width}
            cy={max === min ? height / 2 : height - ((points[points.length - 1] - min) / (max - min)) * height}
            r={3}
            fill={color}
          />
        </>
    ...
    return (
      <Card className="rounded-3xl border-dashed bg-white/80">
        <CardContent className="flex flex-col items-center gap-3 py-12 text-center">
          ...
          <p className="text-sm text-muted-foreground">Be the first to tell others how this dish stacks up.</p>
        </CardContent>
      </Card>
    );
  }
  ...
  return (
    <div className="space-y-4">
      {reviews.map((review) => {
        ...
        return (
          <Card key={review.id} className="rounded-3xl border border-border/40 bg-white/90 shadow-sm">
            <CardContent className="space-y-3 px-5 py-4">
              ...
              <div className="flex flex-wrap items-center gap-3 text-xs text-muted-foreground">
                ...
                <Separator orientation="vertical" className="h-3 bg-border/60" />
                <span>{review.user.first_name ? `${review.user.first_name}'s take` : 'Community voice'}</span>
                {badges.length ? (
                  <>
                    <Separator orientation="vertical" className="h-3 bg-border/60" />
...
export function MealDetailsPage(): JSX.Element {
  ...
  const dietaryBadges = useMemo(
    () => extractMealTags(mealDetails?.tags),
    [mealDetails?.tags]
  );
  ...
  const heroImage =
    ...
    aggregatedReviews.find((review) => Boolean(review.first_image?.image_url))?.first_image?.image_url ??
    null;

  const priceHeadline =
    formatCurrency(priceSummary.latest ?? mealDetails?.avg_price ?? mealDetails?.price ?? null) ?? 'Price not tracked yet';
  const waitHeadline =
  ...
  const canLoadMore = typeof totalReviews === 'number' ? aggregatedReviews.length < totalReviews : false;
    ...
    <div className="flex flex-1 flex-col">
      ...
      >
        ...
        <div className="space-y-0.5">
          ...
          <p className="text-base font-semibold text-foreground">{mealDetails?.name ?? 'Loading…'}</p>
        </div>
      </motion.header>
          ...
          <>
            <section className="overflow-hidden rounded-3xl border border-border/60 bg-white shadow-sm">
              <div className="relative h-[260px] w-full">
                {heroImage ? (
                  <img src={heroImage} alt={mealDetails.name} className="h-full w-full object-cover" loading="lazy" />
                ) : (
                ...
                <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/80 to-transparent p-6 text-white">
                  ...
                  <p className="text-sm text-white/80">{mealDetails.description ?? 'No description provided yet.'}</p>
            ...
            <section className="grid gap-6 lg:grid-cols-3">
              <Card className="rounded-3xl border border-border/60 bg-white/90 shadow-none lg:col-span-2">
                <CardHeader>
                  ...
                  <CardDescription>Track how much diners are paying for this dish over time.</CardDescription>
                </CardHeader>
                <CardContent className="space-y-4">
                  <div className="flex flex-wrap items-center gap-3">
                    <div>
                      <p className="text-2xl font-semibold text-foreground">{priceHeadline}</p>
                      <p className="text-xs uppercase tracking-wide text-muted-foreground">Latest price</p>
                    </div>
                    <Separator orientation="vertical" className="h-10 bg-border/60" />
                    <div>
                      <p className="text-xl font-semibold text-foreground">
                        {priceSummary.average ? formatCurrency(Math.round(priceSummary.average)) ?? '—' : '—'}
                      </p>
                    ...
                    <div>
                      ...
                      <p className="text-xs uppercase tracking-wide text-muted-foreground">Vs first review</p>
                    </div>
                  </div>
                  <TrendSparkline points={priceSummary.points} color="#2563eb" label="Price trend" />
                </CardContent>
              </Card>
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <div className="text-right">
                      <p className="text-lg font-semibold text-foreground">
                        {typeof waitSummary.average === 'number' ? `${Math.round(waitSummary.average)} min` : '—'}
                      </p>
                      <p className="text-xs uppercase tracking-wide text-muted-foreground">Average</p>
                    </div>
                  </div>
                  <TrendSparkline points={waitSummary.points} color="#a855f7" label="Wait-time trend" />
                  <div className="rounded-2xl bg-muted/40 p-3 text-xs text-muted-foreground">
                    Reports come from diner submissions. Share yours when you finish a review to keep this accurate.
                  </div>
                </CardContent>
              </Card>
            </section>
              ...
              <Card className="rounded-3xl border border-border/60 bg-white/90 shadow-none">
                ...
                <CardContent className="space-y-4">
                  ...
                  <div className="rounded-2xl bg-muted/40 p-4">
                    <p className="text-xs uppercase tracking-wide text-muted-foreground">Most-cited tags</p>
                    {reviewTagFrequency.length ? (
                      ...
                      <p className="mt-2 text-sm text-muted-foreground">Reviewers have not flagged anything yet.</p>
                    )}
                  </div>
                  <div className="rounded-2xl border border-dashed border-primary/40 bg-primary/5 p-4 text-sm text-muted-foreground">
                    Add your own dietary callouts when you post a review to improve accuracy for everyone.
                  </div>
                </CardContent>
              </Card>
            </section>
